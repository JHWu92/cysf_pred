...
def up_exp(combos, y, Xs):
    for seed in SEEDS:  # the last seed raise 'Expected n_neighbors <= n_samples,  but n_samples = 5, n_neighbors = 6' in smote.fit_sample
        ...
        up_train_y = upsample(train_y)
        up_train_idx = up_train_y.index
        up_y_dist = up_train_y.round().value_counts().to_dict()
        ...
        for total_or_not, name in combos:
            ...
            # get uped train x and test_x
            X = Xs[total_or_not]
            up_train_x, test_x = X.loc[up_train_idx], X.loc[test_idx]
            feature_names = up_train_x.columns
...
def main():
    ...
    up_exp(combos=combos, y=y, Xs=Xs)
