...
def exp8_one_run(exp_path, Xs, y, train_idx, test_idx):
    ...
    for ftr_combo_name, X in Xs.items():
        ...
        for model_name in MODEL_NAMES:
            ...
            dset = scale_and_selection(train_x, train_y, test_x, test_y, 'None')
            print('feature selection: %d -> %d' % (len(feature_names), dset['selected_ftr'].sum()))
            cv_path = '%s/%s#%s' % (exp_path, ftr_combo_name, 'None')
...
def exp8(Xs, y):
    for seed in SEEDS:
        ...
        exp_path = 'experiment_1001/exp8-spatial/seed_%d' % (seed)
