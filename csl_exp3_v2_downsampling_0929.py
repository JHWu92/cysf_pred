...
def down_exp_one_run(downsample_path, model_names, y, Xs, train_idx, test_idx, seed):
    def grid(model):
        grid_res = grid_cv_a_model(dset['train_x'], dset['train_y'], model, param, kind=kind, name=name,
                                   path=cv_path)
...
def down_exp(model_names, y, Xs):
    for seed in SEEDS:
        ...
        exp_path = 'experiment/up_down_experiment_v2/seed_%d' % seed
...
def main():
    Xs, y = load_data()
    model_names = [('SVM', 'cls'), ('RFcls', 'cls'), ('GDBcls', 'cls')]
