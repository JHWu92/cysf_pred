...
from src.sk_ml import (sk_models, grid_cv_default_params, grid_cv_models, evaluate_grid_cv, evaluator_scalable_cls,
                       model_order_by_speed, show_important_features, confusion_matrix_as_df, bounded_round)
...
def cfsn_imp(df_cv_res, cv_dir, feature_names, test_x, test_y):
    for (kind, name), model in df_cv_res.best_model.iteritems():
        try:
            imp = show_important_features(model, labels=feature_names, set_std=False, show_plt=False).drop('std',
                                                                                                           axis=1)
...
def grid_eval(ds, cv_dir, ftr_name):
    ...
    df_cv_res = grid_cv_models(train_x, train_y, models, params, order=order, path=cv_dir, verbose=True, redo=True, range=(1,5))
...
def baseline_majority_vote(single_pred, test_y, exp_path):
    ...
    res = {'model'           : 'majority_vote',
           'test_f1_weighted': f1_score(bounded_round(test_y, 1, 5), pred, average='weighted'),
           'test_f1_micro'   : f1_score(bounded_round(test_y, 1, 5), pred, average='micro'),
           'test_f1_macro'   : f1_score(bounded_round(test_y, 1, 5), pred, average='macro')}
    df = pd.DataFrame.from_dict([res]).set_index('model')
    df.to_csv('%s/majority_vote.csv' % exp_path)
    return df


def exp_roadnet(ys, train_idx, test_idx, exp_path):
    ...
    train_y, test_y = ys.loc[train_idx], ys.loc[test_idx]
...
def exp_else(ys, train_idx, test_idx, exp_path):
...
def main():
    ...
    maxi, mini = 5,1
    bounded_ys = bounded_round(ys, mini, maxi)
    mode_of_round_y = pd.Series(bounded_ys).mode().values[0]

    for seed in [0, 100, 972, 5258, 7821, 40918, 57852, 168352, 291592, 789729423]:
        exp_path = 'experiment_1107/exp1/seed_%d' % seed
        ...
        baseline_majority_vote(mode_of_round_y, ys.loc[test_idx], exp_path)
...
if __name__ == '__main__':
    CSL_FN = 'data/y_csl_all-2017-11-07.csv'
    main()
