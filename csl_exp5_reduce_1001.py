...
def exp5_reduce_one_run(exp_path, Xs, y, train_idx, test_idx):
    """
...
def exp5_reduce(y, Xs, exp_path):
    for seed in SEEDS:
        # set up experiment path
        sub_exp_path = '%s/seed_%d' % (exp_path, seed)
        mkdirs_if_not_exist(sub_exp_path)
        # get train/test index
        idx_fn = '%s/%s' % (sub_exp_path, 'indices.txt')
        train_idx, test_idx = get_idx(y.index, idx_fn, seed)
        print('====begin one run exp, in sub_exp_path=%s' % sub_exp_path)
        exp5_reduce_one_run(sub_exp_path, Xs, y, train_idx, test_idx)
        # break  # run one seed
...
def main():
    ...
    for fn in glob.glob('data/exp5_ys/1001/*'):
        ...
        exp_path = 'experiment_1001/exp5_reduce/%s#%s' % (frac, sample_seed)
